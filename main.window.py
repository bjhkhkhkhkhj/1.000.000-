...
def switch_screen():
    if btn_answer.text()=="Відповісти":
        qwestion_group.hide()
        answer_group.show()
        btn_aswewr.setText("Наступне питання")
    else:
        qwestion_group.hide()
        answer_group.show()
        btn_aswewr.setText("Відповісти")

btn_menu=QPsuhButton("Меню")
btn_sleep=QPsuhButton("Відпочити")
btn_answer=QPsuhButton("Відповісти")
timer=QSpinBox()
timer.setValue(30)
